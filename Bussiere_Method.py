...
for method in methods:#Goes through methods
    ...
    # Loop through the years to read each CSV file that we have stored in the same working directory
        for country in country_code:#Looping overa available countries
            #filename = f'{country}{year}dom.csv'#Flexible naming convention to allow unqiue combinations.
            filename = f"df_{country}_{year}.csv"
        ...
        for d in demand_type:
            print(f"The demand type that is running now is: {d}")
            ...
            for key,value in dataframes.items():#This loops over the dataframes we have loaded above (e.g. countryyearsdom.csv)
                Matrix_A_dom = np.empty((48,48))#This create an empty 45 x 45 Matrix. This is hte cause because the Supply Use tables come in this form. Should be adjusted appropriately if more industries added or product level supply use table used.
                Matrix_A_dom[:] = np.nan 
                ...
                for i in range(48):
                    for j in range(48):
                        x_ij = df.iloc[i, j + 1]
                        row = df[df['row_series'] == "OUTPUT"].index[0]
                        x_j = df.iloc[row, j + 1]
                ...
                if d == "PC":
                    ...
                    F_d = np.array(df.loc[:47,"HFCE"]+df.loc[:47,"NPISH"])
                elif d == "GC":
                    F_d = np.array(df.loc[:47,"GGFC"])
                elif d == "Inv":
                    F_d = np.array(df.loc[:47,"GFCF"])
                elif d == "EXPO":
                    F_d = np.array(df.loc[:47,"EXPO"])
                else:
                ...
                if d == "PC":
                    F_m = np.array(df.loc[48:95,"HFCE"]+df.loc[48:95,"NPISH"])
                elif d == "GC":
                    F_m = np.array(df.loc[48:95,"GGFC"])
                elif d == "Inv":
                    F_m = np.array(df.loc[48:95,"GFCF"])
                elif d == "EXPO":
                    F_m = np.array(df.loc[48:95,"EXPO"])        
                else:
                    print("Wrong Demand Type")
        ...
        share_csv_data = df_final.to_csv(f'FIGARO_{year}_{method}_new.csv', index = True) 
